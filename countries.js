...
// Function to check if all the neighbours are found
function endRound(){
    const correctChoices = document.querySelectorAll('.correct');
    const correctClickedChoices = document.querySelectorAll('.correct-clicked');
    if(correctChoices.length == correctClickedChoices.length){
        return true;
    }
    else{
        return false;
...
function nextLevel(){
    ...
    document.querySelector(".next-level").removeEventListener('click', nextLevel);
    neighboursFound = 0;
    roundCounter();
    ...
    gameGenerator(countryObjects[randomInt(countryObjects.length)].code3, generateCountries(countryObjects[randomInt(countryObjects.length)].code3));
}

// Function to start a new game with the button
function newGame(){
    round = 1;
    score = 0;
    sideInitial();
...
// Function to show the success image
function endGame(){
...
// Function to generate the entire game
const gameGenerator = (country, countryValues) => {
    cardContainer.innerHTML = "";
    fetch(`https://restcountries.com/v3/alpha/${country}`).then(data=>data.json()).then((data)=>{
        if(data[0].borders==undefined){
            let random = randomInt(countryObjects.length);
            gameGenerator(countryObjects[random].code3, generateCountries(countryObjects[random].code3));
            return;
        }
        // Generate the country card to be found
        cardContainer.innerHTML += `
        <div class="country">
            <img src="https://flagcdn.com/${data[0].cca2.toLowerCase()}.svg" class="country-img">
            <p>${data[0].name.common}</p>
        </div>`;
        // Generate the progress bar
        createProgressBar();
        // Generate the choices
        cardContainer.innerHTML += `<div class="choices"></div>`;
        countryValues.sort(() => Math.random() - 0.5); // Shuffle the array
        ...
        for(let i=0; i<countryValues.length; i++){
            ...
            <div class="country-card">
                <div class="img-space">
                    <img src="${countryValues[i].src}" class="card-img country-img">
                    <div class="text-space">
                        <p>${countryValues[i].name}</p>
                    </div>
                </div>
            </div>`;
        }

        // Add the correct and wrong classes to the choices
        for(let i=0; i<countryValues.length; i++){
            let countryCard = document.querySelectorAll('.country-card')[i];
            if (countryValues[i].isNeighbour == true ){
        ...
        countries.forEach(c => {
            c.addEventListener('click', () => {
                if(c.classList.contains('correct')){
                    ...
                    setTimeout(() => {
                        neighboursFound++;
                        updateProgressBar(country);
                        score+=5;
                        scoreCounter();
                    }, 500);
                    
                } else{
                    c.classList.add('wrong-clicked');
                    score-=3;
                    scoreCounter();
                }
                // Check if all the neighbours are found
                if(endRound()){
                    round++;
...
// Function to generate the array with the country choices
const generateCountries = (country) => {
    let countryValues = [];
    fetch(`https://restcountries.com/v3/alpha/${country}`).then(data=>data.json()).then((data)=>{
        if (data[0].borders==undefined) {
            let choice = randomInt(countryObjects.length);
            generateCountries(countryObjects[choice].code3);
            return ;
        }

        // Push the neighbours to the array
        const borders = data[0].borders;
        //console.log(borders);
        borders.forEach(border => {
            for (let i = 0; i < countryObjects.length; i++) {
                if (countryObjects[i].code3===border) {
                    let countryObj = {};
                    countryObj.name = countryObjects[i].name;
                    countryObj.src = `https://flagcdn.com/${countryObjects[i].code.toLowerCase()}.svg`;
                    countryObj.index = i;
                    countryObj.isNeighbour = true;
                    countryValues.push(countryObj);
                }
            }
        });

        // Push other countries to the array
        while(countryValues.length<12){
            let random = randomInt(countryObjects.length);
            if(countryValues.find(country=>country.index==random) || random==countryObjects.indexOf(data[0])){
                continue;
            } else{
                let countryObj = {};
                countryObj.name = countryObjects[random].name;
                countryObj.src = `https://flagcdn.com/${countryObjects[random].code.toLowerCase()}.svg`;
                countryObj.index = random;
                countryObj.isNeighbour = false;
                countryValues.push(countryObj);
            }
        }
    });
...
const updateProgressBar = (country) => {
    fetch(`https://restcountries.com/v3/alpha/${country}`).then(data=>data.json()).then((data) =>{
        let numOfNeighbours = data[0].borders.length;
        document.querySelector('.progress-complete').style.setProperty("--progress",`${Math.floor((neighboursFound/numOfNeighbours)*100)}%`);
        if(neighboursFound/numOfNeighbours <= 0.3){
            document.querySelector('.progress-complete').style.backgroundColor = "red";
        }
        else if(neighboursFound/numOfNeighbours > 0.3 && Math.floor(neighboursFound/numOfNeighbours) <= 0.6){
            document.querySelector('.progress-complete').style.backgroundColor = "orange";
        }
        else if(neighboursFound/numOfNeighbours > 0.6 && Math.floor(neighboursFound/numOfNeighbours) <= 1){
            document.querySelector('.progress-complete').style.backgroundColor = "green";
        }
    });
}
...
// Initialize the game
const initializer = () => {
    let countryValues = generateCountries("GRE");
    sideInitial();
    gameGenerator("GRE", countryValues);
};
